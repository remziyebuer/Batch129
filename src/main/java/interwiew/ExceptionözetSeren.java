...
public class ExceptionözetSeren {
    /*
    EXCEPTİONS
...
fakat her kırmızı çizgi exceptions değildir.
 "Compile Time Exception" larda ya "throws" veya "try-catch" kullanimali handle edilmeli yoksa
 code calismaz.
...
6) try body 'dan sonra 1 veya daha fazla catch body creat edilebilir. Olusan Exception  hangi catch body
 ile ilgili ise o catch body calisir.  Ayni anda sadece bir catch body calisabilir.
...
Mesela; 12'yi 0'a bolersek Java Exception throw eder. Bu Exception ArithmeticException kullanilarak
 halledilebilir.Run Time Exception'dir.
...
3) ArrayIndexOutOfBoundsException class'i array'lerde olmayan bir index ile islem yapmaya calisirsak
 devreye girer.Run Time Exception'dir.

4) NumberFormatException class'i sayi formatinda olmayan bir String'i parseInt() methodu kullanarak
 integer'a cevirmek istediginizde devreye girer.Run Time Exception'dir.

5) ClassCastException class'i birbirine donusturulemeyen data type'lar birbirine donusturulmek istenirse
devreye girer.
...
Note: catch block'lardan sonra finally block kullanilabilir, finally block'in ozelligi Exception olussada
olusmasada calismasidir.finally block cloud'da yapilan islemlerden sonra cloud ile connection'i kesmek icin
 kullanilir. Cloud ile connection yazdigimiz kod basarili bir sekilde calisirsa isimiz bittigi icin kesilmelidir. Yazdigimiz code
...
AHAN DA 	Interview sorusu : "final", "finally", "finalize" nedir?
	 		 ...
	 		 finally(code block) ==> try-catch block'dan sonra kullanilir ve Exception atilsa da
	 		  atilmasa da calisir.
